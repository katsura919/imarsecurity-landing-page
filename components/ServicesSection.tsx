            ...
            <div className="relative z-10 container-custom">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {services.map((service, index) => (
                        <div
                            key={service.title}
                            className="glass-card rounded-xl p-6 group cursor-pointer transition-all duration-300 hover:border-[rgba(245,158,11,0.4)] hover:-translate-y-1"
                        >
                            {/* Icon */}
                            <div className="w-12 h-12 rounded-lg bg-[rgba(245,158,11,0.1)] flex items-center justify-center mb-4 group-hover:bg-[rgba(245,158,11,0.2)] transition-colors duration-300">
                                <service.icon className="w-6 h-6 text-[var(--accent-yellow)]" />
                            </div>

                            {/* Content */}
                            <h3 className="text-lg font-semibold text-[var(--steel-navy)] mb-2 group-hover:text-[var(--accent-gold)] transition-colors duration-300">
                                {service.title}
                            </h3>
